...
//-------------------------------------------------------------------------------------------------------------------------------------------------
{
	...
	//---------------------------------------------------------------------------------------------------------------------------------------------
	for (DBUser *dbuser in [[DBUser allObjects] sortedResultsUsingKeyPath:FUSER_FULLNAME ascending:YES])
	{
...
//-------------------------------------------------------------------------------------------------------------------------------------------------
{
	...
	//---------------------------------------------------------------------------------------------------------------------------------------------
	[group[FGROUP_MEMBERS] addObjectsFromArray:members];
	[group saveInBackground:^(NSError *error)
	{
		if (error == nil)
		{
			[self loadUsers];
			[Group membersAdded:group members:members];
		}
		else [ProgressHUD showError:@"Network error."];
	}];
}
...
//-------------------------------------------------------------------------------------------------------------------------------------------------
{
	...
	[group saveInBackground:^(NSError *error)
	{
		if (error == nil)
		{
			[Group memberRemoved:group userId:dbuser.objectId];
		}
		else [ProgressHUD showError:@"Network error."];
	}];
}
...
//-------------------------------------------------------------------------------------------------------------------------------------------------
{
	...
	[group saveInBackground:^(NSError *error)
	{
		...
		{
			[ProgressHUD dismiss];
			[Group memberRemoved:group userId:[FUser currentId]];
			//-------------------------------------------------------------------------------------------------------------------------------------
...
//-------------------------------------------------------------------------------------------------------------------------------------------------
{
	NSDictionary *dict = [Chat startGroup2:dbgroup];
	//---------------------------------------------------------------------------------------------------------------------------------------------
	AllMediaView *allMediaView = [[AllMediaView alloc] initWith:dict[@"chatId"]];
...
//-------------------------------------------------------------------------------------------------------------------------------------------------
- (void)didSelectMultiple:(NSArray *)userIds
//-------------------------------------------------------------------------------------------------------------------------------------------------
{
