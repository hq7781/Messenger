...
//-------------------------------------------------------------------------------------------------------------------------------------------------
+ (void)send:(NSString *)chatId recipientId:(NSString *)recipientId groupId:(NSString *)groupId
...
//-------------------------------------------------------------------------------------------------------------------------------------------------
{
	...
	BOOL isPrivate = ([recipientId length] != 0);
	//---------------------------------------------------------------------------------------------------------------------------------------------
	NSPredicate *predicateUser = [NSPredicate predicateWithFormat:@"objectId == %@", recipientId];
	...
	DBGroup *dbgroup = [[DBGroup objectsWithPredicate:predicateGroup] firstObject];
	//---------------------------------------------------------------------------------------------------------------------------------------------
	NSString *senderPicture		= ([FUser picture] != nil) ? [FUser picture] : @"";
	NSString *recipientPicture	= (dbuser.picture != nil) ? dbuser.picture : @"";
	...
	//---------------------------------------------------------------------------------------------------------------------------------------------
	if (isPrivate)	message[FMESSAGE_MEMBERS] = @[[FUser currentId], recipientId];
	if (isGroup)	message[FMESSAGE_MEMBERS] = [dbgroup.members componentsSeparatedByString:@","];
	//---------------------------------------------------------------------------------------------------------------------------------------------
	...
	//---------------------------------------------------------------------------------------------------------------------------------------------
	message[FMESSAGE_RECIPIENTID] = isPrivate ? recipientId : @"";
	message[FMESSAGE_RECIPIENTNAME] = isPrivate ? dbuser.fullname : @"";
	message[FMESSAGE_RECIPIENTINITIALS] = isPrivate ? [dbuser initials] : @"";
	message[FMESSAGE_RECIPIENTPICTURE] = isPrivate ? recipientPicture : @"";
	//---------------------------------------------------------------------------------------------------------------------------------------------
	message[FMESSAGE_GROUPID] = isGroup ? groupId : @"";
	message[FMESSAGE_GROUPNAME] = isGroup ? dbgroup.name : @"";
	message[FMESSAGE_GROUPPICTURE] = isGroup ? groupPicture : @"";
	//---------------------------------------------------------------------------------------------------------------------------------------------
